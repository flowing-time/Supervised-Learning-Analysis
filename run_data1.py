...
def plot_learning_curve(estimator, title, X, y, 
    ...
    axes[1].set_title(f"Data1: Scalability of {title}")

    plt.savefig(f"data1_{title}.png", bbox_inches='tight')
    # plt.show()
...
# %%
plot_validation_curve(KNeighborsClassifier(weights='uniform', n_neighbors=5), 'KNN_p_validation_curve', X, y, param_name='p', param_range=[1, 2, 3, 4, 5])


# %%
knn_clf = KNeighborsClassifier(n_neighbors=1, weights='uniform', p=2)
...
# %%
plot_validation_curve(DecisionTreeClassifier(), 'DT_min_sample_split_validation_curve', X, y, param_name='min_samples_split', param_range=np.arange(2, 1000, 10))


# %%
plot_learning_curve(DecisionTreeClassifier(), 'DT_learning_curve', X, y)
...
# %%
plot_validation_curve(MLPClassifier(solver='adam', hidden_layer_sizes=(100,), max_iter=500), 'ANN_activation_validation_curve', X, y, param_name='activation', param_range=['identity', 'logistic', 'tanh', 'relu'])


# %%
ann_clf = MLPClassifier(solver='adam', hidden_layer_sizes=(100,), activation='logistic', max_iter=500)
...
# %%
plot_learning_curve(AdaBoostClassifier(base_estimator=DecisionTreeClassifier(max_depth=10), n_estimators=50), 'Boost_DT_learning_curve', X, y)
